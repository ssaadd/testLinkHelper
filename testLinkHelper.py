...
class testLinkHelper(QtWidgets.QMainWindow):
    ...
    def init_config(self):
        ...
        # Convert the list data about the test project obtained from testlink into a dictionary in the format {
        # 'project_name': {'id': 12, 'prefix': 'GS'}}
            ...
            for i in range(1, sheet.nrows):
                ...
                if self.is_testcase_exist(case[0]):
                    ...
                    for i in range(len(actions_list) - 1):
                        if i == 0:
                            self.tc.initStep(actions_list[i][0:], expected_results_list[i][0:], MANUAL)
                        else:
                            self.tc.appendStep(actions_list[i][0:],
                                              expected_results_list[i][0:], MANUAL)
                    self.tc.createTestCase(
                        ...
                        preconditions=case_precondition, importance=priority_dict[case[1]],
                        executiontype=executiontype_dict[case[5]], status='7')
    ...
    def _get_suit_info(self, suit_name, proj_info):
        ...
        Check whether a suit exist under the target suit which was specified in target_suit combox.
            if suit exsit, an information list about the suit will be returned;
